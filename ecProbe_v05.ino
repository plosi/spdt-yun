...
unsigned long previous_logging_millis = 0;
...
void parse_command(){
  ...
  if (Bridge.get("log_int", val, sizeof(val))) {
    logging_interval = ((String)val).toInt() * 1000;
  } else if (Bridge.get("continuous_flag", val, sizeof(val))) {
    continuous_flag = atoi(val);
  } else if (Bridge.get("sync_clock", val, sizeof(val))) {
    browser_sec = (String)val;
    setBoardTime(browser_sec);
  } else if (Bridge.get("logging_flag", val, sizeof(val))){
    logging_flag = atoi(val);
  } else {
    return;
  }
  delay(50);
